...
// Keep track of seen comment IDs
const seenComments = {};

// Function to check if a comment has been seen
function isCommentSeen(commentId) {
    return seenComments.hasOwnProperty(commentId);
}

// Function to mark a comment as seen
function markCommentAsSeen(commentId) {
    ...
    comments.forEach(comment => {
        ...
        if (isCommentSeen(commentId)) {
            // Comment has been seen, hide or remove it from the DOM
            comment.style.display = 'none'; // Or comment.remove() to remove it from the DOM
        } else {
